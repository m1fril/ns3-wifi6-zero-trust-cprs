...
std::map<uint32_t, std::string> nodeTypes; // NEW: Maps Node ID to its Type (AP, STA, etc)
...
// NEW: Helper to get the text label for a node
...
void TracePacketReception(std::string context, Ptr<const Packet> p, uint16_t channelFreqMhz,
    ...
    if (packet->GetSize() >= pktSize) {
        ...
        if (csvLogFile.is_open()) {
            ...
            // NEW: Added Node Type to output
            csvLogFile << "WIFI," << nId << "," << GetNodeType(nId) << "," 
                       << Simulator::Now().GetSeconds() << ",RECV\n";
        }
...
void P2PRxDrop(std::string context, Ptr<const Packet> p) {
    if (csvLogFile.is_open()) {
        uint32_t nId = ContextToNodeId(context);
        // NEW: Added Node Type to output
        csvLogFile << "WIRED," << nId << "," << GetNodeType(nId) << "," 
                   << Simulator::Now().GetSeconds() << ",DROP\n";
    }
    std::cout << "\033[1;31m" << Simulator::Now().GetSeconds() << "s: [!!! PACKET LOST !!!] Wired Link Drop at Node " << ContextToNodeId(context) << "\033[0m" << std::endl; 
}
...
void SocketSendTrace(std::string context, Ptr<const Packet> p, const Address& addr) {
    if (csvLogFile.is_open()) {
        uint32_t nId = ContextToNodeId(context);
        // NEW: Added Node Type to output
        csvLogFile << "APP," << nId << "," << GetNodeType(nId) << "," 
                   << Simulator::Now().GetSeconds() << ",SEND\n";
    }
}
...
void AssociationLog(std::string context, Mac48Address address) {
    ...
    if (it == associated.end()) {
        ...
        if (csvLogFile.is_open()) {
            // NEW: Added Node Type to output
            csvLogFile << "ASSOC," << nodeId << "," << GetNodeType(nodeId) << "," 
                       << Simulator::Now().GetSeconds() << ",JOINED\n";
        }
        std::cout << Simulator::Now().GetSeconds() << "s: [ASSOC] Node " << nodeId << " (" << GetNodeType(nodeId) << ") Joined." << std::endl;
    }
#ifdef HAS_NETSIMULYZER
...
class Experiment {
  public:
    ...
    int Run(const WifiHelper& wifi, const YansWifiPhyHelper& wifiPhy,
            ...
            double errorRate, uint32_t queueSize);
};
                ...
                double errorRate, uint32_t queueSize)
{
    ...
    // NEW: Register node types in our dictionary map
    nodeTypes.clear();
    nodeTypes[gatewayNode.Get(0)->GetId()] = "Gateway";
    for(uint32_t i=0; i < apNodes.GetN(); i++) {
        nodeTypes[apNodes.Get(i)->GetId()] = "AP";
    }
    for(uint32_t i=0; i < staNodes.GetN(); i++) {
        nodeTypes[staNodes.Get(i)->GetId()] = "STA";
    }
    ...
    Config::Connect("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Mac/MacTxDrop", MakeCallback(&WifiMacDropTrace));
    ...
    if (enableVisualization)
    {
        ...
        jsonName << "Bianchi-" << nAps << "APs-" << networkSize << "STAs-" << queueSize << "Q-" << pktIntervalUs << "Int.json";
    ...
    csvName << "Bianchi-" << nAps << "APs-" << networkSize << "STAs-" << queueSize << "Q-" << pktIntervalUs << "Int.csv";
    
    csvLogFile.open(csvName.str(), std::ios::out);
    if (csvLogFile.is_open()) {
        std::cout << "[INFO] Generating CSV Output: " << csvName.str() << std::endl;
        // NEW: Updated Header to include NodeType
        csvLogFile << "Event,NodeID,NodeType,Time_s,Action\n";
    }
...
{
    // UPDATED DEFAULTS FOR LONGER SIMULATION
    uint32_t nMinStas = 50; 
    uint32_t nMaxStas = 50; 
    uint32_t nStepSize = 5;
    uint32_t nAps = 1;      
    double duration = 60.0;   // Increased to 60 seconds to see sustained drops
    bool pcap = false;      
    bool infra = true;      
    std::string phyMode = "OfdmRate54Mbps";
    uint16_t guardIntervalNs = 800;
    
    uint32_t pktInterval = 100; // Fast interval (100us) to force congestion
    double errorRate = 0.0;      
    uint32_t queueSize = 20;    // Small queue to ensure we see drops quickly
    ...
    for (uint32_t n = nMinStas; n <= nMaxStas; n += nStepSize) {
        // Pass the 'duration' variable here
